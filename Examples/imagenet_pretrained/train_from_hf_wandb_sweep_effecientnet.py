"""
Training script for EfficientNet models with WandB sweep support.

Supports two model variants:
- efficientnet-b4-perforated-fc: EfficientNet-B4 with perforated final classifier
- efficientnet-b5: EfficientNet-B5 baseline (non-perforated)

No HuggingFace weights are used; torchvision ImageNet pretrained weights are used.

Usage:
python train_from_hf_wandb_sweep_effecientnet.py --model efficientnet-b4-perforated-fc --dataset flowers102
python train_from_hf_wandb_sweep_effecientnet.py --model efficientnet-b5 --dataset flowers102
"""
...
def get_dataset_config(dataset_name):
    ...
    NOTE: This script uses torchvision ImageNet pretrained weights.
    """
    configs = {
        "flowers102": {
            "num_classes": 102,
...
def _get_dataset_targets(dataset):
    """Extract class labels from common torchvision dataset target fields."""
    for attr_name in ("targets", "labels", "_labels"):
        if hasattr(dataset, attr_name):
            ...
            if len(targets) == len(dataset):
                return targets
    raise ValueError(
        f"Could not find dataset targets for {dataset.__class__.__name__}. "
        "Expected one of: targets, labels, _labels"
    )


def class_balanced_split(dataset, seed=42):
    """Split dataset into class-balanced val/test subsets.

    For each class, this uses floor(count/2) samples in val and floor(count/2) in test.
    If a class has an odd count, one sample from that class is dropped to keep an equal
    class count in both subsets.
    """
    ...
    for class_id in sorted(class_to_indices.keys()):
        ...
        per_split = len(shuffled_indices) // 2
        val_indices.extend(shuffled_indices[:per_split])
        test_indices.extend(shuffled_indices[per_split : 2 * per_split])
        dropped_count += len(shuffled_indices) - (2 * per_split)

    if dropped_count > 0:
        print(
            f"Dropped {dropped_count} samples to keep per-class val/test counts equal"
        )
...
def load_model(model_name, num_classes, perforate=False, dataset_name=None):
    ...
    if model_name == "efficientnet-b4-perforated-fc":
        model = torchvision.models.efficientnet_b4(
            weights=EfficientNet_B4_Weights.IMAGENET1K_V1
        )
        print("Loaded torchvision EfficientNet-B4 with ImageNet pretrained weights")

        if perforate:
            print("Configuring PAI to perforate only the final classifier layer...")
            ...
            GPA.pc.set_module_ids_to_track([".features", ".avgpool"])
            ...
            if (
                ...
                save_name = f"efficientnet_b4_fc_{num_classes}cls"
            ...
            print(
                f"Model perforated successfully (classifier layer only) - save_name: {save_name}"
            )
        ...
        print("Loaded torchvision EfficientNet-B5 with ImageNet pretrained weights")

    else:
        raise ValueError(f"Unknown model: {model_name}")

    # EfficientNet uses classifier[1] as the final Linear layer.
    if not hasattr(model, "classifier"):
        raise ValueError(f"Cannot adapt model - expected classifier layer for {model_name}")

    if isinstance(model.classifier, nn.Sequential):
        in_features = model.classifier[-1].in_features
        new_head = nn.Linear(in_features, num_classes)

        if perforate and model_name == "efficientnet-b4-perforated-fc":
            from perforatedai import utils_perforatedai as UPA
            from perforatedai.modules_perforatedai import PAINeuronModule

            model.classifier[-1] = PAINeuronModule(new_head, "classifier.1")
            print(
                f"Replaced classifier layer for {num_classes} classes and converted to PAINeuronModule"
            )
...
def train_single_run(args, train_loader, val_loader, test_loader, num_classes):
    ...
    perforate = args.model in ["efficientnet-b4-perforated-fc"]
...
def get_model_name_from_index(model_index):
    ...
    Args:
        model_index: Integer from 0-1

    Returns:
        Model name string
    """
    model_mapping = {
        0: "efficientnet-b4-perforated-fc",
        1: "efficientnet-b5",
    }
...
def get_sweep_config(dataset_name):
    ...
    if dataset_name == "flowers102":
        ...
        base_config["parameters"] = {
            ...
            "weight_decay": {"values": [1e-5, 1e-4, 5e-4, 1e-3]},
            "label_smoothing": {"values": [0.0, 0.05, 0.1]},
            "scheduler_mode": {"values": [0, 1]},  # 0=CosineAnnealing, 1=ReduceLROnPlateau
            # Dendritic hyperparameters (used by efficientnet-b4-perforated-fc)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train EfficientNet models with WandB sweep support"
    )
    parser.add_argument(
        "--sweep-dataset",
        type=str,
    ...
    parser.add_argument(
        "--wandb-project",
        default="efficientnet-transfer-learning",
        type=str,
    ...
    if args.sweep_dataset or args.sweep_id:
        if args.sweep_id and args.sweep_id != "main":
            ...
            print(
                f"  python train_from_hf_wandb_sweep_effecientnet.py --sweep-id {sweep_id} --wandb-project {project_name}{entity_arg}"
            )
