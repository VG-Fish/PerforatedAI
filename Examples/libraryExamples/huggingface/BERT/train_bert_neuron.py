#!/usr/bin/env python
"""
Baseline BERT Classification Training Script on AWS Trainium (without PAI integration)

This script trains a Transformer model (RoBERTa or BERT) on either an IMDB
or SNLI dataset using AWS Trainium/Neuron. All configuration is provided via 
command‑line arguments. It supports a DSN flag (--dsn) which, when enabled, sets 
the number of encoder layers to 0 (i.e. uses only embeddings, sum pooling, and classifier).

Requirements:
  - torch_neuronx (Beta 3 stack: torch 2.11+, torch_neuronx 2.11+)
  - transformers
  - datasets

Usage example for IMDB (uses all defaults - no arguments required):
    python train_bert_neuron.py
       
Usage example for SNLI (only specify parameters that differ from IMDB defaults):
    python train_bert_neuron.py --dataset snli --batch_size 256 --max_len 128 \
       --model_save_location ./model_output_SNLI_neuron
"""
...
def main():
    ...
    parser.add_argument("--dsn", type=str2bool, nargs='?', const=True, default=True, help="Enable DSN mode (set number of encoder layers to 0)")
    ...
    parser.add_argument("--model_save_location", type=str, default="./model_output_IMDB_neuron", help="Directory to save the trained model")
    parser.add_argument("--early_stopping", type=str2bool, nargs='?', const=True, default=True, help="Enable early stopping")
    ...
    # Use bf16 for Neuron (recommended for Trainium), fp16 for CUDA
    use_bf16 = NEURON_AVAILABLE
    use_fp16 = torch.cuda.is_available() and not NEURON_AVAILABLE
    
    training_args = TrainingArguments(
        output_dir=args.model_save_location if args.model_save_location else "./results_neuron",
        num_train_epochs=args.num_epochs,
        per_device_train_batch_size=args.batch_size,
        per_device_eval_batch_size=args.batch_size,
        warmup_steps=int(1000 / args.batch_size) if not args.reduce_lines_for_testing else 1,
        weight_decay=0.01,
        logging_dir="./logs_neuron",
        ...
        bf16=use_bf16,
        fp16=use_fp16,
        run_name=f"run_neuron_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    # Start training.
    print("Starting training on Trainium/Neuron...")
    trainer.train()
