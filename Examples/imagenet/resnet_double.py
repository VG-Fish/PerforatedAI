...
# Custom ResNet class with configurable channel widths
class ResNetCustom(nn.Module):
    def __init__(
        self,
        block: type[Union[BasicBlock, Bottleneck]],
        layers: list[int],
        channels: list[int],  # Custom channel configuration [c1, c2, c3, c4]
        num_classes: int = 1000,
        zero_init_residual: bool = False,
        groups: int = 1,
        width_per_group: int = 64,
...
# Method 1: Thin ResNet - Reduced width, same depth
# ResNet18 with 0.625x width: [40, 80, 160, 320] channels
# ~4.6M parameters (vs 11.7M for ResNet18)
def resnet18_thin(**kwargs: Any) -> ResNetCustom:
    """ResNet-18 with reduced width (0.625x channels).
    
    Architecture: [2, 2, 2, 2] blocks with [40, 80, 160, 320] channels
    Parameters: ~4.6M (39% of ResNet18)
    """
    return ResNetCustom(BasicBlock, [2, 2, 2, 2], [40, 80, 160, 320], **kwargs)


# Method 2: Shallow ResNet - Reduced depth, same width  
# ResNet with [1, 1, 1, 1] blocks and standard [64, 128, 256, 512] channels
# ~5.8M parameters (vs 11.7M for ResNet18)
def resnet10_shallow(**kwargs: Any) -> ResNetCustom:
    """ResNet-10 with standard width but reduced depth.
    
    Architecture: [1, 1, 1, 1] blocks with [64, 128, 256, 512] channels
    Parameters: ~5.8M (50% of ResNet18)
    """
    return ResNetCustom(BasicBlock, [1, 1, 1, 1], [64, 128, 256, 512], **kwargs)


# Method 3: Balanced ResNet - Moderate width and depth reduction
# Custom architecture with [1, 2, 1, 1] blocks and [48, 96, 192, 384] channels
# ~3.7M parameters (vs 11.7M for ResNet18)
def resnet12_balanced(**kwargs: Any) -> ResNetCustom:
    """ResNet-12 with balanced width and depth reduction.
    
    Architecture: [1, 2, 1, 1] blocks with [48, 96, 192, 384] channels
    Parameters: ~3.7M (32% of ResNet18)
    """
    return ResNetCustom(BasicBlock, [1, 2, 1, 1], [48, 96, 192, 384], **kwargs)
...
class ResNetPAI(nn.Module):
    ...
    def _make_layer_pb(self, other_block_set, other_resnet, block_id):
        ...
        for i in range(len(other_block_set)):
            ...
            if block_type == BasicBlock or block_type == resnet_pt.BasicBlock:
                layers.append(other_block_set[i])
            elif block_type == Bottleneck or block_type == resnet_pt.Bottleneck:
                layers.append(other_block_set[i])
            else:
                print(
