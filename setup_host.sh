...
#
# AUTOMATIC COMMIT: the first time you complete setup inside the container,
# this host auto-saves the image as 'my-neuron-setup'. Every login after that
# skips ALL the slow work (pull, extraction, driver install, pip installs).
# You never have to run 'docker commit' yourself.
#
...
# Common mount flags for the container.
run_container() {
...
# CASE 1: We're already on the committed image — nothing new to save.
# Launch straight in, no watcher, no re-commit.
if [ "$RUN_IMAGE" = "$COMMITTED_IMAGE_NAME" ]; then
  step "launch container (committed image — instant, no re-commit needed)"
  log "Inside the container, run: /host-script/setup_docker.sh"
  CONTAINER_NAME="neuron-work-$(date +%s)"
  set +e
  run_container "$CONTAINER_NAME"
  RUN_RC=$?
  set -e
  log "Container '$CONTAINER_NAME' exited (rc=$RUN_RC). Remove with: docker rm $CONTAINER_NAME"
  exit 0
fi

# CASE 2: First-time (uncommitted) path. Launch a named container and start a
# background watcher that auto-commits as soon as setup_docker.sh signals done.
step "launch container (first run — will AUTO-SAVE image when setup completes)"
CONTAINER_NAME="neuron-work-$(date +%s)"
COMMIT_SIGNAL="$LOG_DIR/READY_TO_COMMIT"
rm -f "$COMMIT_SIGNAL"

log "Launching container '$CONTAINER_NAME'."
log ">>> Inside the container, run:  /host-script/setup_docker.sh"
log ">>> When that finishes, this host AUTO-SAVES the image as '${COMMITTED_IMAGE_NAME}'."
log ">>> You do NOT need to run 'docker commit'. Next login will be instant."

# Background watcher (host side): wait for the in-container signal, commit once.
(
  set +e
  # Wait for the container to actually come up.
  for _ in $(seq 1 60); do
    docker inspect "$CONTAINER_NAME" >/dev/null 2>&1 && break
    sleep 1
  done
  # Poll for the signal file. Bail out if the container disappears first.
  while [ ! -f "$COMMIT_SIGNAL" ]; do
    docker inspect "$CONTAINER_NAME" >/dev/null 2>&1 || exit 0
    sleep 3
  done
  if docker inspect -f '{{.State.Running}}' "$CONTAINER_NAME" 2>/dev/null | grep -q true; then
    echo "[auto-commit] setup signal received — saving '${COMMITTED_IMAGE_NAME}' ..." | tee -a "$LOG_FILE"
    if docker commit "$CONTAINER_NAME" "$COMMITTED_IMAGE_NAME" >>"$LOG_FILE" 2>&1; then
      echo "[auto-commit] DONE. Future logins skip all setup." | tee -a "$LOG_FILE"
    else
      echo "[auto-commit] FAILED — see $LOG_FILE. Manual fallback: docker commit $CONTAINER_NAME ${COMMITTED_IMAGE_NAME}" | tee -a "$LOG_FILE"
    fi
  fi
  rm -f "$COMMIT_SIGNAL"
) &
...
# Launch interactively. Don't let the ERR trap fire on the shell's exit code.
set +e
run_container "$CONTAINER_NAME"
RUN_RC=$?
set -e

# Container exited — stop the watcher.
kill "$WATCHER_PID" 2>/dev/null || true
wait "$WATCHER_PID" 2>/dev/null || true

if docker image inspect "$COMMITTED_IMAGE_NAME" >/dev/null 2>&1; then
  log "SUCCESS: '${COMMITTED_IMAGE_NAME}' is saved. From now on, just run ./setup_host.sh — it's instant."
else
  log "NOTE: '${COMMITTED_IMAGE_NAME}' was NOT saved (setup likely didn't complete inside the container)."
  log "Re-run ./setup_host.sh, then run /host-script/setup_docker.sh to completion to trigger the auto-save."
fi
log "Container '$CONTAINER_NAME' exited (rc=$RUN_RC). It's left in place; remove with: docker rm $CONTAINER_NAME"
