...
def random_crop(image, label, crop_range):
    ...
    crop_ratio = float(crop_ratio)  # Convert numpy type to Python float
    ...
    i, j = int(i), int(j)  # Convert numpy types to Python int for indexing
...
def rotate_90(image, label, prob):
    if prob:
        ...
        k = int(k)  # Convert numpy type to Python int
        if k:
...
def rotate_angle(image, label, angle_max):
    if angle_max:
        ...
        angle = float(
            angle
        )  # Convert numpy type to Python float for OpenCV compatibility
...
# ------------------------------------------------------------------------------
def resize_image(
    image, expected_size, pad_value, ret_params=False, mode=cv2.INTER_LINEAR
):
    """
