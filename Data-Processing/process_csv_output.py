...
def _extract_model_id_from_run_name(run_name: str) -> Optional[str]:
    """Extract model_id from run name strings that include model_index_<n>."""
...
def _collect_per_run_best_scores_by_model(
    df: pd.DataFrame,
    dendrite_columns: Sequence[str],
    model_name_map: Dict[str, str],
) -> pd.DataFrame:
    """Collect per-run best val/test scores for each model, ignoring dendrite count."""
    ...
    for _, row in df.iterrows():
        model_id = _extract_model_id_from_run_name(row.get("run_name", ""))
        if model_id is None:
            continue

        model_cols = model_metric_columns.get(model_id, {"val": [], "test": []})
        if not model_cols["val"] and not model_cols["test"]:
            continue

        best_val = pd.to_numeric(row[model_cols["val"]], errors="coerce").max() if model_cols["val"] else np.nan
        best_test = pd.to_numeric(row[model_cols["test"]], errors="coerce").max() if model_cols["test"] else np.nan
        ...
        rows.append(
            {
                ...
                "best_val_score": float(best_val) if not pd.isna(best_val) else np.nan,
                "best_test_score": float(best_test) if not pd.isna(best_test) else np.nan,
            }
        )

    if not rows:
        raise ValueError("Could not compute per-run best model scores from CSV data.")

    return pd.DataFrame(rows)


def _create_bell_curve_best_scores_plot(
    run_best_scores_df: pd.DataFrame,
    output_path: str,
    base_model_zero_scores: Optional[Dict[str, List[float]]] = None,
    base_model_zero_label: str = "Base model / d0",
) -> None:
    """Create a bell-curve-style chart comparing model best val/test score distributions."""
    model_ids = sorted(
        run_best_scores_df["model_id"].dropna().unique(),
        key=lambda m: int(m.split("_")[1]) if re.match(r"model_\d+$", str(m)) else 9999,
    )
    if not model_ids:
    ...
    for ax, (metric_col, metric_title) in zip(axes, metric_specs):
        ...
        metric_values_all = run_best_scores_df[metric_col].dropna().astype(float).tolist()
        global_bins = None
        if metric_values_all:
            base_bins = max(5, min(15, int(math.sqrt(len(metric_values_all))) + 2))
            doubled_bins = max(10, min(40, base_bins * 2))
            global_bins = np.histogram_bin_edges(metric_values_all, bins=doubled_bins)

        for idx, model_id in enumerate(model_ids):
            subset = run_best_scores_df[run_best_scores_df["model_id"] == model_id]
            ...
            if global_bins is not None:
                ax.hist(values, bins=global_bins, density=True, alpha=0.2, color=color)
            else:
            ...
            if len(values) >= 2:
                ...
                if std_v > 1e-12:
                    ...
                    y_vals = (1.0 / (std_v * math.sqrt(2.0 * math.pi))) * np.exp(
        ...
        # Draw a dedicated base-model dendrite-0 histogram distribution as requested.
        if base_model_zero_scores is not None:
            metric_key = "val" if metric_col == "best_val_score" else "test"
            base_values = [float(v) for v in base_model_zero_scores.get(metric_key, []) if not pd.isna(v)]
            if base_values:
                any_drawn = True
                if global_bins is not None:
                    ax.hist(
                ...
                # Also draw the fitted bell-curve line for d0 to match other series.
                if len(base_values) >= 2:
                    ...
                    if base_std > 1e-12:
                        ...
                        if x_max == x_min:
                            ...
                            x_max += 1.0
                        x_vals = np.linspace(x_min, x_max, 200)
                        y_vals = (1.0 / (base_std * math.sqrt(2.0 * math.pi))) * np.exp(
                            -((x_vals - base_mean) ** 2) / (2.0 * (base_std ** 2))
                        )
                        ax.plot(x_vals, y_vals, color="black", linewidth=2.2, linestyle="--")
                    else:
                        ax.axvline(base_values[0], color="black", linewidth=2.2, linestyle="--")
                else:
                    ax.axvline(base_values[0], color="black", linewidth=2.2, linestyle="--")
...
def _collect_base_model_zero_scores(
    df: pd.DataFrame,
    base_model_id: str,
) -> Dict[str, List[float]]:
    """Collect raw score lists for base model dendrite-0 val/test columns."""
    out: Dict[str, List[float]] = {"val": [], "test": []}
    val_col = f"{base_model_id}_dendrite_0_max_val"
    test_col = f"{base_model_id}_dendrite_0_max_test"

    if val_col in df.columns:
        out["val"] = pd.to_numeric(df[val_col], errors="coerce").dropna().astype(float).tolist()
    if test_col in df.columns:
        out["test"] = pd.to_numeric(df[test_col], errors="coerce").dropna().astype(float).tolist()

    return out


def _create_base_model_d0_vs_final_histogram(
    df: pd.DataFrame,
    base_model_id: str,
    output_path: str,
    ...
    # Limit to runs belonging to the selected base model when run_name is available.
    base_rows_df = df
    if "run_name" in df.columns:
        model_ids = df["run_name"].apply(_extract_model_id_from_run_name)
        matched = df[model_ids == base_model_id]
        if not matched.empty:
            base_rows_df = matched

    # Use run-level aggregation to avoid counting multiple dendrite rows from one run.
...
def _write_best_run_per_model_csv(
    ...
    """Write one row per model: run name + val/test scores for best run by selected metric."""
    ...
    for model_id in model_ids:
        ...
        # Select by requested metric. Fall back to the opposite metric if needed.
        primary_col = "best_val_score" if select_metric == "val" else "best_test_score"
        fallback_col = "best_test_score" if select_metric == "val" else "best_val_score"
    ...
    output_name = (
        "best_run_scores_by_model_best_val.csv"
        if select_metric == "val"
        else "best_run_scores_by_model_best_test.csv"
    )
    output_path = os.path.join(output_dir, output_name)
    pd.DataFrame(rows_out, columns=["model_name", "run_name", "val_score", "test_score"]).to_csv(
...
def main() -> None:
    ...
    try:
        ...
        created_files.append(_write_companion_csv_for_png(bell_curve_plot_path, run_best_scores_df))

        base_d0_vs_final_hist_path = os.path.join(output_dir, "hist_base_d0_vs_final.png")
        base_hist_paths, base_hist_df = _create_base_model_d0_vs_final_histogram(
            df,
            base_model_id_for_hist,
            base_d0_vs_final_hist_path,
            model_name_map=model_name_map,
        )
        for hist_path in base_hist_paths:
            created_files.append(hist_path)
            created_files.append(_write_companion_csv_for_png(hist_path, base_hist_df))
        ...
        for metric_key, metric_label, metric_columns in metric_runs:
            ...
            stats_df["param_count"] = stats_df["column"].map(param_counts_by_column)
            ...
            created_files.append(_write_companion_csv_for_png(scatter_by_param_count_plot_path, scatter_rows))
